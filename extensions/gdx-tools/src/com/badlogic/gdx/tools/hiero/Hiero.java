...
public class Hiero extends JFrame {
    ...
    void open(File file) {
        ...
        for (Object o : settings.getEffects()) {
            ConfigurableEffect settingsEffect = (ConfigurableEffect) o;
    ...
    void exit(final int exitCode) {
        rendererCanvas.stop();
        EventQueue.invokeLater(() -> System.exit(exitCode));
    }

    private void initializeEvents() {
        fontList.addListSelectionListener(evt -> {
        ...
        sampleTextRadio.addActionListener(evt -> glyphCachePanel.setVisible(false));
        glyphCacheRadio.addActionListener(evt -> glyphCachePanel.setVisible(true));
        ...
        effectsList.addListSelectionListener(evt -> {
        ...
        addEffectButton.addActionListener(evt -> new EffectPanel((ConfigurableEffect) effectsList.getSelectedValue()));
        ...
        saveMenuItem.addActionListener(evt -> {
            ...
            } else if (!lastOpenFilename.isEmpty()) {
        ...
        exitMenuItem.addActionListener(evt -> dispose());

        sampleNeheButton.addActionListener(evt -> {
            sampleTextPane.setText(NEHE_CHARS);
            resetCacheButton.doClick();
        });

        sampleAsciiButton.addActionListener(evt -> {
            StringBuilder buffer = new StringBuilder();
        ...
        sampleExtendedButton.addActionListener(evt -> {
