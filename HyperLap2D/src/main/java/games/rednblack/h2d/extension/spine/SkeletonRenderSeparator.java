...
public class SkeletonRenderSeparator {
    ...
    /**
     * Renders the specified skeleton. If the batch is a PolygonSpriteBatch, {@link #draw(PolygonSpriteBatch, Skeleton, SlotRange)} is
     * called. If the batch is a TwoColorPolygonBatch, {@link #draw(TwoColorPolygonBatch, Skeleton, SlotRange)} is called. Otherwise the
     ...
     * next.
     */
    public void draw(Batch batch, Skeleton skeleton, SlotRange slotRange) {
        if (batch instanceof TwoColorPolygonBatch) {
            draw((TwoColorPolygonBatch) batch, skeleton, slotRange);
            return;
        }
        if (batch instanceof PolygonSpriteBatch) {
            draw((PolygonSpriteBatch) batch, skeleton, slotRange);
            return;
        }
        ...
        for (int i = slotRange.start, n = slotRange.end; i < n; i++) {
            ...
            if (attachment instanceof RegionAttachment) {
                ...
                if (attachmentSkeleton != null) draw(batch, attachmentSkeleton, slotRange);
            }
        }
    }
     ...
     * next.
     */
    public void draw(PolygonSpriteBatch batch, Skeleton skeleton, SlotRange slotRange) {
        if (batch == null) throw new IllegalArgumentException("batch cannot be null.");
        if (skeleton == null) throw new IllegalArgumentException("skeleton cannot be null.");
     ...
     * next.
     */
    public void draw(TwoColorPolygonBatch batch, Skeleton skeleton, SlotRange slotRange) {
        if (batch == null) throw new IllegalArgumentException("batch cannot be null.");
        if (skeleton == null) throw new IllegalArgumentException("skeleton cannot be null.");
