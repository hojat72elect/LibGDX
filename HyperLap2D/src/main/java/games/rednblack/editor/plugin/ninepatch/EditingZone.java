...
public class EditingZone extends Actor {
    ...
    private static final Color GUIDE_COLOR = new Color(1.0f, 94f / 255f, 0f / 255f, 0.5f);
    private static final Color OVER_GUIDE_COLOR = new Color(1.0f, 173f / 255f, 125f / 255f, 1f);
    ...
    private final float[] splitPositions = new float[4];
    private final int[] splits = new int[4];
                ...
                if (selectedSplit >= 0) {
                    ...
                    if (selectedSplit == 1) {
                        ...
                        if (texture.getRegionWidth() - splits[1] < splits[0]) {
                            int tmp = splits[0];
                            splits[0] = texture.getRegionWidth() - splits[1];
                            splits[1] = texture.getRegionWidth() - tmp;
                            selectedSplit = 0;
                            mouseOverSplit = selectedSplit;
                        }
                        splitUpdate();
                    ...
                    if (selectedSplit == 2) { // top
                        ...
                        if (texture.getRegionHeight() - splits[2] < splits[3]) {
                            int tmp = splits[2];
                            splits[2] = texture.getRegionHeight() - splits[3];
                            splits[3] = texture.getRegionHeight() - tmp;
                            selectedSplit = 3;
                            mouseOverSplit = selectedSplit;
                        }
                        splitUpdate();
                    ...
                    if (selectedSplit == 3) {
                        ...
                        if (splits[3] > texture.getRegionHeight() - splits[2]) {
                            int tmp = splits[3];
                            splits[3] = texture.getRegionHeight() - splits[2];
                            splits[2] = texture.getRegionHeight() - tmp;
                            selectedSplit = 2;
                            mouseOverSplit = selectedSplit;
                        }
                        splitUpdate();
    ...
    public void drawSplits(Batch batch, float parentAlpha) {
        ...
        if (mouseOverSplit == 0) shapeRenderer.setColor(overColor);
        else shapeRenderer.setColor(guideColor);
        shapeRenderer.line(getX() + splitPositions[0], getY(), getX() + splitPositions[0], getY() + getHeight());

        if (mouseOverSplit == 1) shapeRenderer.setColor(overColor);
        else shapeRenderer.setColor(guideColor);
        shapeRenderer.line(getX() + splitPositions[1], getY(), getX() + splitPositions[1], getY() + getHeight());

        if (mouseOverSplit == 2) shapeRenderer.setColor(overColor);
        else shapeRenderer.setColor(guideColor);
        shapeRenderer.line(getX(), getY() + splitPositions[2], getX() + getWidth(), getY() + splitPositions[2]);

        if (mouseOverSplit == 3) shapeRenderer.setColor(overColor);
        else shapeRenderer.setColor(guideColor);
