...
class ContainerPadBottomLmlAttributeTest {

    private lateinit var attribute: TestableContainerPadBottomLmlAttribute
    ...
    @Before
    fun setUp() {
        attribute = TestableContainerPadBottomLmlAttribute()
    ...
    @Test
    fun `applyToCell sets padBottom on cell from actor`() {
        val padValue = 20f
        `when`(actor.padBottom).thenReturn(padValue)

        attribute.publicApplyToCell(actor, cell)

        verify(cell).padBottom(padValue)
    }

    class TestableContainerPadBottomLmlAttribute : ContainerPadBottomLmlAttribute() {
