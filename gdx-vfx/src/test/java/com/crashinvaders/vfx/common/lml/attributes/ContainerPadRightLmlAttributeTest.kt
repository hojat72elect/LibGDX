...
class ContainerPadRightLmlAttributeTest {

    private lateinit var attribute: TestableContainerPadRightLmlAttribute
    ...
    @Before
    fun setUp() {
        attribute = TestableContainerPadRightLmlAttribute()
    ...
    @Test
    fun `applyToCell sets padRight on cell from actor`() {
        val padValue = 20f
        `when`(actor.padRight).thenReturn(padValue)

        attribute.publicApplyToCell(actor, cell)

        verify(cell).padRight(padValue)
    }

    class TestableContainerPadRightLmlAttribute : ContainerPadRightLmlAttribute() {
